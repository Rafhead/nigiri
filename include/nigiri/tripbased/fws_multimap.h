...
struct fws_multimap_entry {
  ...
  using iterator = typename tripbased::vector<T>::const_iterator;

  fws_multimap_entry(tripbased::vector<T> const& data,
                     tripbased::vector<Index> const& index,
                     Index key)
      : data_(data),
  ...
  tripbased::vector<T> const& data_;
...
struct fws_multimap {
  ...
  tripbased::vector<T> data_;
  tripbased::vector<Index> index_;
...
struct shared_idx_fws_multimap {
  ...
  explicit shared_idx_fws_multimap(tripbased::vector<Index> const& base_index)
  ...
  tripbased::vector<Index> const& base_index_;
  tripbased::vector<T> data_;
};

template <typename T, typename Index = uint64_t>
struct nested_fws_multimap {
  explicit nested_fws_multimap(tripbased::vector<Index> const& base_index)
      : base_index_(base_index) {}
